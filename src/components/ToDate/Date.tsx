...
export default function Date({ month, day, date }: DateProps) {
    ...
    let [toMonth, toDate]: string[] = nowTime.split("-");
    ...
    React.useEffect(() => { //컴포넌트가 마운트 되고난 후
        setDisplayMonth(months[Number(toMonth)]);
        setDisplayDay(days[toDay]);
        setDisplayDate(Number(toDate));
    }, [month, day, date]);

    return (
        <>
            <div className={styles.dateTitle}>TO-DAYS</div>
